...
import TopClubs from "@/components/widget/StakePage/mobile/TopClubs";
...
export default function StakeContent() {
  ...
  const handleChangePage = (clubId: number) => {
    setIsChangePage(!isChangePage)
    setSelectClubId(clubId)
  }
  if (loading) {
    return <Loading loading={loading} className={''} />
  }

  return (!isChangePage ? <>
    <StakeStatistic statData={data?.data?.coindata}></StakeStatistic>
    <TopClubs />
    <StakeContStyled className="mt-10">
