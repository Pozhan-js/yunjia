...
const Item = ({ item = {} }: any) => {
    ...
    return (<div className={'ml-[1rem] flex items-center cursor-pointer'} onClick={() => {
        navigate(`/clubs/${item?.club_id}`)
    }}>
        <img src={item?.img} className={'w-[2.58rem] h-[2.58rem]'} />
        <div className={'flex flex-col ml-[0.65rem]'}>
            <p className={'text-white text-[1rem] font-gtwalm'}>{item?.name}</p>
