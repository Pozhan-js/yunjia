...
const TypeContent = ({ data, type }) => {
    if (type == 1) {
        return (
            <>
                <RacingMatch_ list={data?.coming} />
                <RacingResult list={data?.ended} />
            </>
        )
    } else if (type == 2) {
        return (
            <RacingMatch_ list={data?.coming} showTitle={false} />
        )
    } else if (type == 3) {
        return (
            <RacingResult list={data?.ended} showTitle={false} />
        )
    }
}
...
function NoticeModal() {
    ...
    const { run, loading } = useRequest(getRacingNotice,
        {
            ...
            manual: true,
            onSuccess: (tdata) => {
                console.log('NoticeModal mobile,getRacingNotice:', tdata);
                setData(tdata?.data);
    ...
    const handleNoticeHidden = (tdata) => {
        ...
        if (tdata?.data.coming.length > 0) {
            ...
            console.log('getRacingNotice coming date:', tdata?.data.coming[0].enddate, endDate)
        } else if (tdata?.data.ended.length > 0) {
            endDate = new Date(tdata?.data.ended[0].enddate);
            console.log('getRacingNotice ended date:', tdata?.data.ended[0].enddate, endDate)
        }
        let now = new Date();
